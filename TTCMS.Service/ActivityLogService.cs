...
namespace TTCMS.Service
{
    ...
    public class ActivityLogService : IActivityLogService
    {
        ...
        public IPagedList<ActivityLog> ListActivityLog(Page page, string search)
        {
            if (search != "")
            {
                return activityLogRepository.GetPageDESC(page, x => x.User.UserName.Contains(search) || x.ServiceName.Contains(search), order => order.ExecutionTime);
            }
            return activityLogRepository.GetPageDESC(page, x => true, order => order.ExecutionTime);
        }
